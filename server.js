...
app.use(
  '/repository',
  express.static(repository),
  serveIndex(repository, { icons: true })
)
...
app.post('/', (req, res) => {
  ...
  let result = injector
    .inject(repository, req.body.chain, req.body.address, files)
    .then(result => {
      ...
      // ? `Found ${
      // result.length
      // } other addresses of this contract: ${result.join(', ')}<br/>`
      // : '') +
